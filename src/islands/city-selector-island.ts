/**
 * City Selector Island - Handles geolocation + city change events
 */

const STORAGE_KEY = 'iftar-saati-city';
const DEFAULT_CITY_ID = '9541'; // İstanbul

// Inline city coordinates for nearest-city lookup
// (simplified version - only used client-side for geolocation)
const CITY_COORDS: Array<{ id: string; name: string; lat: number; lng: number }> = [
  ...
  { id: '9252', name: 'Aydın', lat: 37.856, lng: 27.8416 },
  ...
  { id: '9311', name: 'Bitlis', lat: 38.401, lng: 42.1095 },
  { id: '9315', name: 'Bolu', lat: 40.736, lng: 31.6061 },
  { id: '9327', name: 'Burdur', lat: 37.7203, lng: 30.2908 },
  { id: '9335', name: 'Bursa', lat: 40.1885, lng: 29.061 },
  ...
  { id: '9432', name: 'Elazığ', lat: 38.681, lng: 39.2264 },
  { id: '9440', name: 'Erzincan', lat: 39.75, lng: 39.5 },
  ...
  { id: '9594', name: 'Kars', lat: 40.6167, lng: 43.1 },
  ...
  { id: '9807', name: 'Sakarya', lat: 40.694, lng: 30.4358 },
  { id: '9819', name: 'Samsun', lat: 41.2928, lng: 36.3313 },
  { id: '9839', name: 'Siirt', lat: 37.9333, lng: 41.95 },
  ...
  { id: '9522', name: 'Iğdır', lat: 39.888, lng: 44.0048 },
  { id: '9935', name: 'Yalova', lat: 40.65, lng: 29.2667 },
...
function findNearestCity(lat: number, lng: number): { id: string; name: string } {
...
function dispatchCityChanged(cityId: string, cityName: string) {
...
function setCity(cityId: string) {
  ...
  // Update banner
  const bannerCity = document.getElementById('banner-city-text');
  if (bannerCity) bannerCity.textContent = city.name;

  // Dispatch event
  dispatchCityChanged(cityId, city.name);
}
...
async function init() {
  ...
  try {
    ...
    // Geolocation failed or denied - silent fallback
  }

  // 2. Try localStorage
  if (!cityId) {
    cityId = localStorage.getItem(STORAGE_KEY);
  }
